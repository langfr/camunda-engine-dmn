...
public class DmnDecisionContextImpl implements DmnDecisionContext {
  ...
  protected DmnDecisionResult evaluateDecisionTable(DmnDecisionTable decisionTable, Map<String, Object> variables) {
    ...
    for (DmnRule rule : decisionTable.getRules()) {
      if (isRuleApplicable(rule, variables, inputs, evaluationCache)) {
        DmnDecisionTableRuleImpl matchingRule = evaluateMatchingRule(rule, variables, inputs, evaluationCache);
  ...
  protected DmnDecisionTableRuleImpl evaluateMatchingRule(DmnRule rule, Map<String, Object> variables, Map<String, DmnDecisionTableValue> inputs, Map<String, Object> evaluationCache) {
    DmnDecisionTableRuleImpl matchingRule = new DmnDecisionTableRuleImpl();
    matchingRule.setKey(rule.getKey());
    Map<String, DmnDecisionTableValue> ruleOutputs = evaluateRuleOutput(rule, variables, inputs, evaluationCache);
    matchingRule.setOutputs(ruleOutputs);
    return matchingRule;
  }

  protected Map<String, DmnDecisionTableValue> evaluateRuleOutput(DmnRule rule, Map<String, Object> variables, Map<String, DmnDecisionTableValue> inputs, Map<String, Object> evaluationCache) {
    ...
    for (DmnClauseEntry conclusion : rule.getConclusions()) {
      ...
      Object value = evaluateExpression(conclusion, variables);
