...
public class FeelOutputEntryHandler extends AbstractDmnClauseHandler<OutputEntry> {

  protected void postProcessExpressionText(DmnElementHandlerContext context, OutputEntry expression, DmnExpressionImpl dmnExpression) {
    if (hasJuelExpressionLanguage(dmnExpression) && !isExpression(dmnExpression)) {
      String expressionText = dmnExpression.getExpression();
      dmnExpression.setExpression("${" + expressionText + "}");
    }
  }
